...
def get_question():
    ...
    if st.session_state.question_type == 0 and st.session_state.show_question: #population
        ...
        st.session_state.current_proposals = generate_population_question(st.session_state.country_name, error_margin, country_population_dict)
    ...
    if st.session_state.question_type == 1 and st.session_state.show_question: #capital
        ...
        st.session_state.current_proposals, st.session_state.d = generate_country_capital_question(st.session_state.country_name, country_list, country_capital_dict)
    ...
    if st.session_state.question_type == 2 and st.session_state.show_question: #flag
        ...
        st.session_state.current_proposals, st.session_state.d = generate_flag_question(st.session_state.country_name, country_list)    
    ...
    if st.session_state.question_type == 3 and st.session_state.show_question: #QG
        ...
        st.session_state.current_question, st.session_state.current_proposals, st.session_state.d = get_abstract_question_from_index(game_data, idx, st.session_state.s2v_model, st.session_state.sentencizer, st.session_state.ner_model)
...
idx = st.number_input("index dans le CSV", min_value = 0, max_value = len(game_data)-1)
...
if st.session_state.show_question : 
    ...
    if st.session_state.question_type == 1 : #Capitals
        ...
        for i in range(2):
            ...
            c2.write(str(2*i+1) + ". " + st.session_state.current_proposals[2*i+1])
        
        st.session_state.user_input = st.number_input("Answer (index of solution)", min_value = 0)
...
if st.session_state.show_answer:
    ...
    if st.session_state.question_type == 0 : #Population
        ...
        st.write(evaluate_population_question(st.session_state.current_proposals, st.session_state.user_input), f"Correct population was : {st.session_state.current_answer} !")

    if st.session_state.question_type == 1 : #Capitals
        st.session_state.current_answer = game_data["capital_name"][idx]
        st.write(evaluate_country_capital_question(st.session_state.current_proposals, st.session_state.d, st.session_state.user_input), f"Correct capital was : {st.session_state.current_answer} !")

    if st.session_state.question_type == 2: #Flags
        st.write(evaluate_country_flag_question(st.session_state.current_proposals, st.session_state.d, st.session_state.user_input), f"Correct flag was : ")
        st.image(Image.open("country_flags/" + st.session_state.country_name + "_flag.png"))

    if st.session_state.question_type == 3 : #QG
        for key in st.session_state.current_proposals :
            if st.session_state.d[key]:
                st.session_state.current_answer = key
        st.write(evaluate_abstract_question(st.session_state.current_proposals, st.session_state.d, st.session_state.user_input), f"Correct answer was : {st.session_state.current_answer} !")
